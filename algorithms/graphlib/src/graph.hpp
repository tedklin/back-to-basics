...
There exists an auxiliary "Edge" class (see mst.hpp), which also represents the
concept of an edge in a graph, but this is only used for defining MSTs and not
for defining Graphs themselves.
