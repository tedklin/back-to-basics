...
void vertex_set_constructor() {
  Vertex v1("A"), v2("B"), v3("C");
  std::unordered_set<Vertex> vertex_set({v1, v2, v3});

  // TODO: why doesn't this work? isn't there an implicit conversion from string
  // to Vertex, as defined by the Vertex constructor?
  // std::unordered_set<Vertex> vertex_set_conv({"A", "B", "C"});

  Graph graph(vertex_set, false);
  std::cout << graph.vertex_set_str() << "\n\n";
}

void unweighted_al_constructor() {
  Vertex v1("A"), v2("B"), v3("C");

  // These two graphs should be the same when the graph is undirected, but
  // different when the graph is directed.
...
// The example in the comment at the top of graph.hpp.
...
int main() {
  ...
  // hash_overload();
  // vertex_set_constructor();
  // unweighted_al_constructor();
  example();
}
