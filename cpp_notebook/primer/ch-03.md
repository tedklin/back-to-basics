...
- vector operations
	- ***push_back*** takes a value and "pushes" it as a new last element onto the "back" of the vector (appends value to vector).
		- **note that a [range for loop](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-05.md#control-structures) must not change the size of the sequence over which it is iterating.**
...
- **loops that use iterators should usually not add or remove elements to the container to which the iterators refer.**
	...
	- if you have to do this, [use caution](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-09.md).
...
- built-in arrays are a compound type
	- array declarators are of form *name[dim]*
		- **dimension must be a [constant expression](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-02.md#const-qualifier-24) greater than zero.**
- initialization
