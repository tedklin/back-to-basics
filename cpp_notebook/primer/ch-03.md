...
- strings default initialize to the empty string.
- **initialization forms** (general, not string-specific)
	- ***copy initialization*** has an = sign and uses the right-side of the equal sign as the (single) initializing value.
		- results in [implicit class-type conversion](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-07.md#additional-class-features), which library classes might allow (i.e. std::string does for string literals), but i think it introduces too much unknown behavior for using classes i define myself.
	- ***direct initialization*** (shown below with parentheses) can be used with initializers that consist of one or more values.
		- can also be used with brackets (see example with [vector](#library-vector-type-33))
		- **stick with this form of initialization!**
			- however, keep in mind [they can lead to the "most vexing parse" disambiguity problem.](https://en.cppreference.com/w/cpp/language/direct_initialization)
~~~
// how i initialize a string (all direct initializations)
...
- processing strings
	...
	- [**range-based *for* loop**](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-05.md#control-structures) is good for processing every character in a string. (C++11)
...
- vector initialization
	...
	- **list initialize** using { } curly braces (direct or copy initialization)
		- if we use braces, but there is no way for the compiler to use the initializers to list initialize the object, the compiler would then try to use those initializers to construct the object (as if they were inside parentheses).
			- **ensure that our bracketed list initializer actually performs list initialization!** (when that is our goal)
	- direct initialize from a specified number of elements with each element [value initialized](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-02.md#variables-22).
		- we also have the option of providing a default initial value for all elements.
	- direct initialize from iterators that point to a convertible element type (includes built-in array iterators, see section below on arrays)
~~~
// how i initialize a vector (all direct initializations)
...
vector<T> v6(begin(otherContainer), end(otherContainer)); // v6 is a vector corresponding to a separate container
~~~
...
- use [range for loops](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-05.md#control-structures) wherever possible to avoid possible subscripting bugs.
...
- initialization
	- **arrays follow [default initialization](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-02.md#variables-22) rules for built-in types**
	...
	- character arrays can be initialized from a string literal.
		- includes the **null character '\0'** at the end of every string literal!
