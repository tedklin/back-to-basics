...
- ***this***
	- member functions are called on behalf of an object. (exception: static functions?)
	- member functions access the object on which they were called through an implicit const pointer (to the object) named *this*.
	...
	- we can also return a dereferenced *this* to return the object as a whole from a function.
		- recall from ch6, **use a reference function-return-type to return the object (*&ast;this*) as an lvalue.** (return the object itself, not a temporary copy of the object)
...
- **default constructor**
	- classes control default initialization through the default constructor, which takes no arguments (usually has no parameters).
		- a constructor that supplies default arguments for all its parameters also defines default constructor.
	- if a class does not explicitly define **any** constructor at all, the compiler implicitly defines a ***synthesized default constructor***.
		...
		- *synthesized* default constructors are not automatically implicitly defined if a class has other constructors.
	- if we want to use the *synthesized* default constructor **with** other constructors, we can write ***= default*** after the parameter list on either the default constructor declaration or definition (C++11).
- defining a constructor outside the class body follows the rules for any other member function defined outside.
- ***constructor initializer list*** 
	...
	- when any data member is **omitted** from the constructor initializer list, the data member is implicitly initialized using the same process as the synthesized default constructor.
...
struct Sales_data {
	...
	Sales_data(unsigned num, double price):
		units_sold(num), revenue(price*num) { }
	
	// in-class initializers
	unsigned units_sold = 0;
	double revenue = 0;
	
	// this ends up default-initialized, which is only ok because we know strings default initialize to empty.
...
### misc conventions and tips
- **routinely use the constructor initializer list!**
	- generally good idea to write constructor initializers in the same order as the corresponding data members are declared.
	- **avoid using members to initialize other members** when possible, as the order of initialization depends on unobvious factors (see above).
- the best way to specify default values for data members of class type (and built-in types?) is with in-class initializers. (C++11)
- constructors should not override in-class initializers except to provide a different initial value.
- **request / rely on the synthesized default constructor only if you know it does the right thing.**
	- when all built-in and compound-type data members have in-class initializers, and members of other class-types have in-class intializers or <ins>known</ins> default constructors, etc.
...
- **(nonmember) function friendship**
	...
	- friend functions (nonmember and member) don't become members of the class "granting friendship".
		- friend declarations can appear anywhere inside the class body, unaffected by access control.
...
### misc conventions and tips
- as a matter of style, use *struct* for classes intending for all of its members to be public and use *class* if we intend to have any private members.
- in general, *public* is related to interfacing and *private* is related to implementation.
- group friend declarations together at the beginning or end of a class body.
...
- **type members** ([local type aliases](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-02.md#dealing-with-types-25)) can be defined by a class.
	- subject to the same access controls as any other member (public indicates that we want users to use that type name).
...
- **every class defines its own new scope.**
	- the name of a member defined outside the class body must include a scope prefix (***className::***) indicating the class it belongs to.
		...
		- once the compiler sees the prefix with the scope operator in the function name, the rest of the definition is interpreted as being inside the scope of the class (we can access other members normally without extra qualifications).
...
- **name lookup for class member *declarations*:**
	...
	- when a declaration inside a class *uses* a name (i.e. as a type), the compiler first looks inside the class (before / above the name usage) for the declaration *for* that name.
...
## defining abstract data types (7.1)
