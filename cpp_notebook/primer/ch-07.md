...
- **stick with in-class initializers to specify default values for data members of class type (and built-in types).** (C++11)
...
- **declare single-argument constructors with the keyword *explicit* to suppress [implicit class-type conversions](#additional-class-features)!**
...
- **implicit class-type conversions (*bad!*)**
	...
	- **bottom line (at least for now as a beginner): do not allow implicit class-type conversions!!!**
		- **declare single-argument constructors with the prefix keyword *explicit* to suppress implicit conversions.**
