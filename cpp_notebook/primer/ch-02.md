...
- initialization
	...
	- default initialization
		- without an explicit initializer, built-in type variables defined outside any function body are initialized to zero, defined inside a function are uninitialized.
		- classes control how we initialize objects of that class type. most classes let us define objects without explicit initializers (like std::string)
...
- pointer usage
	...
	- *void** pointers can hold the address of objects of any type, but cannot be dereferenced.
...
constant expressions (skimmed, refer to Primer for details)
...
- we can ask the compiler to verify that a variable is a constant expression by using the *constexpr* keyword in its declaration. (C++11)
	- can only use on literal types (arithmetic types, references, and pointers)
	- imposes a top-level const, so order of declaration in some cases would look different from previous rules.
