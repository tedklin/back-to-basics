...
- initialization
	...
	- **initialization forms** (pulled from ch13)
		...
		- ***copy initialization***
			...
			- when we use copy initialization, we are asking the compiler to copy the right-hand operand into the object being created, converting that operand if necessary.
				- that conversion might be an [implicit class-type conversion](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-07.md#additional-class-features), which depends on the *explicit*-ness of the corresponding constructor.
