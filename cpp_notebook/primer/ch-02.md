...
- initialization
	...
	- ***list initialization*** (C++11)
		...
		- the compiler will not let us *list initialize* variables of built-in type if the initializer might lead to the loss of information (*int* type object being list-initialized with a *long double* initializer)
	- ***default initialization***
		- **without an explicit initializer, built-in type variables defined inside a function are uninitialized (with undefined value).** 
			- defined outside any function body are initialized to zero.
		- classes control how we initialize objects of that class type. most classes let us define objects without explicit initializers
			- for example, *std::string* default initializes to empty string no matter where it's defined.
	- ***value initialization***
		- *NOTE: i'm not sure about technical details about when value initialization happens, most information here is from other other chapters.
		- generally happens when we only supply a size to something like a vector or array to be initialized.
			- note that an array (built-in) will still be *default initialized* if we don't provide an initializer at all! value initialization happens for arrays when we add a list initializer; see [ch3](https://github.com/tedklin/pseudoblog/blob/master/cpp_notebook/primer/ch-03.md#built-in-arrays-35) for details.
		-  the library implicitly creates a value-initialized element initializer for each element. (**zero for built-in types**, element-value-initialized array for built-in arrays(?), **default initializer for class types like string**)
...
- not really sure about the details of initialization.